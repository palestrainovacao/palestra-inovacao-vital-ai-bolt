...
interface DiaperUsageFilterControlsProps {
  searchTerm: string;
  setSearchTerm: (value: string) => void;
  filterResident: string;
  setFilterResident: (value: string) => void;
  filterShift: 'all' | 'morning' | 'afternoon' | 'night';
  setFilterShift: (value: 'all' | 'morning' | 'afternoon' | 'night') => void;
  filterPeriod: '7' | '30' | '90' | 'all';
  setFilterPeriod: (value: '7' | '30' | '90' | 'all') => void;
  filterDate: string;
  setFilterDate: (value: string) => void;
  sortBy: 'date' | 'resident' | 'quantity' | 'shift';
  setSortBy: (value: 'date' | 'resident' | 'quantity' | 'shift') => void;
...
export const DiaperUsageFilterControls: React.FC<DiaperUsageFilterControlsProps> = ({
  searchTerm,
  setSearchTerm,
  filterResident,
  setFilterResident,
  ...
  const handleSortChange = (field: 'date' | 'resident' | 'quantity' | 'shift') => {
    if (sortBy === field) {
      toggleSortDirection();
    } else {
      setSortBy(field);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex space-x-2">
              ...
              <button
                onClick={() => handleSortChange('shift')}
                className={`px-3 py-1 text-xs rounded-full flex items-center space-x-1 ${
                  sortBy === 'shift' 
                    ? 'bg-blue-100 text-blue-700' 
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
